...
@section('content')
        ...
        <div class="box-header with-border">
            <h3 class="box-title">Διαχείριση Παρατηρητών Διαιτησίας</h3>
        ...
        <div class="box-body">
            <div class="table-responsive">
                <table id="ref_observer-table" class="table table-condensed table-hover">
                    <thead>
                    <tr>
                        <th>Επώνυμο</th>
                        <th>Όνομα</th>
...
<div id="ref_observer-modal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h3 id="termsLabel" class="modal-title">Παρατηρητής Διαιτησίας-Λεπτομέρειες</h3>
      </div>
...
@section('after-scripts')
    ...
    <script>
        $(function () {
            ...
            $('#ref_observer-table').DataTable({
                processing: false,
                serverSide: true,
                autoWidth: false,
                stateSave: true,
                ajax: {
                    url: '{{ route("admin.file.ref_observer.get") }}',
                    type: 'post',
        ...
        $('#ref_observer-table').on ('click', '.modal-trigger', function(){
          id= $(this).attr('load');
          $.get(id).done(function(data){
              $('.modal-body').html(data);
              $('#ref_observer-modal').appendTo('body').modal('show');
          }
            );
