@if(isset($grades))
{{Form::text('referee-grade-'.$match, $ref_grades , ['class'=>'form-control', 'rel'=> 'referee', 'match'=> $match, 'readonly'=> $readonly])}}
<small id="gcehelp" class="form-text text-muted">{{$lastname}} {{$firstname}}</small>	
@else
	{{Form::text('referee-'.$match, $lastname.' '.$firstname , ['class'=>'form-control referee', 'rel'=> 'referee', 'match'=> $match, 'readonly'=> $readonly])}}
	{{Form::hidden('ref_id-'.$match, $id , ['class'=>'form-control ', 'id'=>'ref_id-'.$match])}}
	<div id="info-{{ $match }}">
		<table class="table table-responsive">
			...
			<tr>
				<td id="ref_days-{{ $match }}-referee">{!! $ref_days !!}</td>
				<td id="refBlock-{{ $match }}-referee">{!! $refBlock !!}</td>
				<td id="teamBlock-{{ $match }}-referee">{!! $teamBlock !!}</td>
				<td id="sameTime-{{ $match }}-referee">{!! $sameTime !!}</td>
				<td id="save-{{ $match }}-referee"></td>
			</tr>
			<tr>
				<td>
					<center><i class="fa fa-users" data-toggle="tooltip" data-html="true" data-placement="top"  data-original-title="Γηπεδούχος" style="color: #0000FF"></i></center>
				</td>
				<td id="team1-{{ $match }}-referee">
				</td >
				<td>
					<center><i class="fa fa-users" data-toggle="tooltip" data-html="true" data-placement="top"  data-original-title="Φιλοξενούμενος" style="color: #FF0000"></i></center>
				</td>
				<td id="team2-{{ $match }}-referee"  colspan="2">
				</td>
			</tr>
			<table class="table table-responsive">
				<tr>
