...
@section('page-header')
    <h1>
        {{ app_name() }}
        <small>Ορισμοί Παρατηρητών</small>
    </h1>
@endsection
...
<div class="box box-success">
        <div class="box-header with-border">
             <h3 class="box-title">Ορισμός Παρατηρητών Ανά Ημερομηνία</h3>
...
@section('after-scripts')
   {{ Html::script("https://cdn.datatables.net/v/bs/dt-1.10.15/datatables.min.js") }}
        ...
        $(document).ready(function () {
             $('#selectAll-publ').click(function(){
                ...
                $('.publ').each(function(index, element){
                  ...
                  var stat=(check)?1:0;
                            $.ajax( {
                                  url: "{{ route('admin.orismos.observer.savePubl') }}",
                                  data: {
                                    match_id: match,
                                    status: stat
                                  },
    ...
    var oTable= $('#match-table').dataTable({
                ...
                stateSave: true,
                ajax: {
                    url: '{{ route("admin.orismos.observer.getDate") }}',
                    type: 'post',
                ...
                columns: [
                    ...
                    {data: 'observer', name: 'observer', sortable: false},
    ...
    var $cur_date_time= '';
     ...
     $(document).on('focus','.observer', function(){
                $(".observer").autocomplete({
                          source: function( request, response ) {
                                              $.ajax( {
                                                url: "{{ route('admin.file.observer.getObserver') }}",
                                                dataType: "json",
                                                data: {
                                                  term: request.term
                                                },
                          ...
                          select: function(event, ui) {
                            ...
                            /* Check if observer is at another match*/
                            $.ajax( {
                                  url: "{{ route('admin.orismos.observer.checkOtherMatch') }}",
                                  data: {
                                    id: ui.item.id,
                                    match_id: match
                                  },
                            ...
                            /* assigns observer to match*/
                            $.ajax( {
                                  url: "{{ route('admin.orismos.observer.save') }}",
                                  data: {
                                    id: ui.item.id,
                                    match_id: match,
                                    kind: rel
